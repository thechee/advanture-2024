...
export const TripCard = ({ booking }) => {
  ...
  return (
    <li
      className="trip-card"
      style={cancelled ? { backgroundColor: "#F4F4F4" } : {}}
      onClick={() => navigate(`${booking.id}`)}
    >
      <div className="trip-card-info">
        <p style={cancelled ? { textDecorationLine: "line-through" } : {}}>
          {dateRange}
        </p>
        <h4>
          {booking.vanInfo.make} {booking.vanInfo.model} {booking.vanInfo.year}
        </h4>
        <p>
          {booking.vanInfo.city}, {booking.vanInfo.state}
        </p>
        <p>{booking.status}</p>
        {!cancelled ? (
          <div className="trip-card-owner">
            <img
              className="trip-card-owner-img"
              src={booking.ownerInfo.profileImage}
              alt=""
            />
            <p>
              {booking.ownerInfo.firstName} {booking.ownerInfo.lastName[0]}.
            </p>
          </div>
        ) : (
